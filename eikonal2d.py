"""
Operators to perform 2D Eikonal tomography hi dan
...
@jit(nopython=True, cache=True)
def _fmm_tt_lin_fwd_inv(delta_v: np.ndarray, delta_tt: np.ndarray, vv: np.ndarray, tt: np.ndarray, tt_idx: np.ndarray, dx: float, dz: float):
    """Fast-marching method linearized forward inverse (A delta_TT = delta_v)"""
    nx = delta_v.shape[0]
    nz = delta_v.shape[1]
    ns = np.array([nx, nz])
    drxns = [-1, 1]
    ...
    for idx_t0 in tt_idx:
        ...
        if tt[idx_t0[0] + idrx[0], idx_t0[1]] > tt[idx_t0[0], idx_t0[1]]:
            ...
            if abs(denom) > 0.0:
                delta_v_scaled[idx_t0[0], idx_t0[1]] += delta_tt[idx_t0[0], idx_t0[1]] * denom - idrx[1] * 2.0 * fdt0[1] * delta_tt[
        ...
        # Using single stencil along x direction to update value
            ...
            if abs(denom) > 0.0:
                delta_v_scaled[idx_t0[0], idx_t0[1]] += delta_tt[idx_t0[0], idx_t0[1]] * denom - idrx[0] * 2.0 * fdt0[0] * delta_tt[
                    idx_t0[0] + idrx[0], idx_t0[1]] 
        else:
            denom = - 2.0 * (idrx[0] * fdt0[0] + idrx[1] * fdt0[1])
            if abs(denom) > 0.0:
                delta_v_scaled[idx_t0[0], idx_t0[1]] += delta_tt[idx_t0[0], idx_t0[1]] * denom - idrx[0] * 2.0 * fdt0[0] * delta_tt[
                    idx_t0[0] + idrx[0], idx_t0[1]] - idrx[1] * 2.0 * fdt0[1] * delta_tt[idx_t0[0], idx_t0[1] + idrx[1]]
    return
...
def _solve_linearized_fwd_inv(vel0: np.ndarray, delta_v: np.ndarray, ishot: int, dx: float, dz: float, tt0: np.ndarray, tt_idx: np.ndarray, RecPos: np.ndarray):
    """Function to solve linearized problem inverse"""
    # Sorting traveltime in ascending order
    tt_idx = _sorting2D(tt0, tt_idx)
    delta_tt = np.zeros_like(vel0)
...
def _solve_linearized_adj(vel0: np.ndarray, data: np.ndarray, ishot: int, dx: float, dz: float, tt0: np.ndarray, tt_idx: np.ndarray, SouPos, RecPos: np.ndarray):
    # delta_tt = np.zeros_like(vel0)
    ...
    source_loc_gaussian=torch.tensor(gaussian_function(XX, ZZ, np.asarray(list(SouPos)).flatten()/100))
    rec_loc_gaussian=torch.tensor(gaussian_function(XX, ZZ, np.asarray(list(RecPos)).flatten()/100))
    x = torch.from_numpy(np.expand_dims(np.transpose(np.asarray([np.float32(vel0), np.float32(source_loc_gaussian),np.float32(rec_loc_gaussian)])), axis=0))

    print(x.shape)
    fno_model.eval()
    out_sample1 = fno_model(x)
    print (out_sample1.shape)
    # print(out_sample1)
    # print(out_sample1.shape())

    return out_sample1.detach().numpy()[0,:,:,0]
...
class EikonalTT_lin_2D(o.Operator):
    ...
    def adjoint(self, add, model, data):
        ...
        for shot in range(self.ns):
            print (model.shape)
            model[:] += _solve_linearized_adj(self.vel[:], data[:], shot, self.dx, self.dz, self.tt_maps[shot], self.tt_idx, self.SouPos, self.RecPos[shot])
        return
